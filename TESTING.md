...
- **`tests/project-detail.spec.js`** — clicking `.portfolio-more-info` opens the project-detail overlay for the active project (via keyboard too); it closes via the close button, Escape, and backdrop click; focus moves in on open and returns to the trigger on close; Tab never reaches page content behind the overlay; description/links are omitted when empty and rendered when present; the overlay renders one image per entry in the project's `images` array (3 for Fintrack, today's real data); the image carousel's arrows/dots are hidden for single-image entries and navigate correctly (arrow click + dot click) for Fintrack; the image carousel autoplays on its own after ~4s; the video carousel is hidden with today's real no-video data.
